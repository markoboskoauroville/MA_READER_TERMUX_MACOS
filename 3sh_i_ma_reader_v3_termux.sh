...
###############################################################################
# MA READER TERMUX  (Edge / Speechify)  -  installer for Termux   edition: v3.48
#
...
banner_fire() {
  ...
  printf '   %sR E A D E R%s  %sv3.48%s\n' "$KEY" "$OFF" "$VIOLET" "$OFF"
...
def load_state():
    ...
    # two decimals, not zero: a pinch leaves the size between two steps,
...
<main>
  ...
  <section class="view hidden" id="helpView">
    <div class="help">
      ...
      <p class="sub">MA Reader <span id="appVer">v3.48 &middot; Edge / Speechify</span></p>
...
/* A size that is not a number used to arrive exactly as it was found. The
...
function step(kind, d){
  if(kind==="speed"){
    ...
    /* A pinch can leave the size between two steps. A+ and A- put it back
...
function boot(){
  ...
  Promise.all([
    ...
    ST.loop = !!st.loop;
    ST.size = sizeOf(st.size, 13); ST.autoplay = (st.autoplay!==false); ST.focus = !!st.focus;
